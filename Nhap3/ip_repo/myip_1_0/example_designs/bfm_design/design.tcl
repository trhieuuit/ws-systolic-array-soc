proc create_ipi_design { offsetfile design_name } {
	...
	make_bd_intf_pins_external  [get_bd_intf_pins myip_0/S01_AXIS]
	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins myip_0/S01_AXIS_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins myip_0/S01_AXIS_ARESETN]
	make_bd_intf_pins_external  [get_bd_intf_pins myip_0/M00_AXIS]
	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins myip_0/M00_AXIS_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins myip_0/M00_AXIS_ARESETN]
...
set_property -name {xsim.simulate.runtime} -value {100ms} -objects [get_filesets sim_1]
	...
	set offset_file "${bd_path}/myip_tb_include.svh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef myip_tb_include_vh_"
	puts $fp "`define myip_tb_include_vh_\n"
...
set test_bench_file ${ip_path}/example_designs/bfm_design/myip_tb.sv
...
remove_files -quiet -fileset sim_1 myip_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top myip_tb [get_filesets sim_1]
