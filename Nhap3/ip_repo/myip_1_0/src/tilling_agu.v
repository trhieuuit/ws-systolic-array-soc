...
module tiling_agu #(
    ...
    always @(*) begin
        ...
        case (state_r)
            IDLE_S:       if (start_pulse) next_state_r = FETCH_W_S; // Dùng start_pulse thay vì start_i
            ...
            UPDATE_CNT_S: begin
                if (m_cnt_r >= dim_m_i - SYS_N && n_cnt_r >= dim_n_i - SYS_N && k_cnt_r >= dim_k_i - SYS_N)
                    next_state_r = WAIT_DONE_S; // Chuyển sang chờ xả ống thay vì về IDLE
                else
                    next_state_r = FETCH_W_S;
            end
            WAIT_DONE_S: next_state_r = IDLE_S; // Đứng đây 1 nhịp Clock cho d2 ghi xong BRAM rồi mới về IDLE
...
// ==========================================
    ...
    always @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) done_r <= 0;
            ...
            done_r <= 1; // Bật cờ khi BRAM C ĐÃ GHI XONG
        else if (start_i == 0)
            done_r <= 0; // CHỈ HẠ CỜ khi CPU Linux đọc xong và ép start_i về 0
    end
